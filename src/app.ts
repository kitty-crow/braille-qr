...
import type { Meta, Res } from "./types.js";
...
export class App {
  ...
  async run(argv: readonly string[]): Promise<Res> {
    ...
    const data = cfg.fmt === "html"
      ? this.pg.make(code, cfg, meta)
      : this.tty.make(code, cfg, cfg.out === undefined && process.stdout.isTTY === true);
  ...
  private async emit(fmt: "html" | "term", path: string | undefined, data: string): Promise<string | undefined> {
